...
class EfficientSam(nn.Module):
    ...
    def predict_masks(
        ...
        batched_points = batched_points.to(torch.float32)
    ...
    def get_rescaled_pts(self, batched_points: torch.Tensor, input_h: int, input_w: int):
        ...
        # 确保 input_h 和 input_w 是标量
        if isinstance(input_h, torch.Tensor):
            # 如果是张量，取第一个元素作为标量值
            input_h = input_h[0].item() if input_h.numel() > 0 else input_h.item()
        if isinstance(input_w, torch.Tensor):
            # 如果是张量，取第一个元素作为标量值
            input_w = input_w[0].item() if input_w.numel() > 0 else input_w.item()
        ...
        # 计算缩放因子
        scale_x = img_size / input_w
        scale_y = img_size / input_h

        return torch.stack(
            [
                torch.where(
                    batched_points[..., 0] >= 0,
                    batched_points[..., 0] * scale_x,
                    torch.tensor(-1.0, dtype=torch.float32, device=batched_points.device),
                ),
                torch.where(
                    batched_points[..., 1] >= 0,
                    batched_points[..., 1] * scale_y,
                    torch.tensor(-1.0, dtype=torch.float32, device=batched_points.device),
                ),
            ],
            dim=-1,
